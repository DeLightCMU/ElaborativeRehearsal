...
def main():
  ...
  _model = zeroshot.models.ervse.PrecompERVSEModelLFHelper(config)
    
  _dataset_fn = zeroshot.datasets.prevideo.ZSLPrecompERVideoLabelWordDataset
  
  if opts.is_train:
    yaml.dump({'config': framework.run_utils.edict2dict(config)},
      open(os.path.join(saver_cfg.log_dir, 'config.yaml'), 'w'))
    ...
    trn_dataset = _dataset_fn(data_cfg.name_files['trn'], data_cfg.ft_dir,
      ...
      video_concept_file=data_cfg.video_concept_file, cat_cpt_defn=data_cfg.cat_cpt_defn,
