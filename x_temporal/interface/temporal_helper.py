...
from x_temporal.utils.log_helper import init_log, add_log_file_handler, get_log_format
...
class TemporalHelper(object):
  def __init__(self, config, ckpt_dict=None, inference_only=False):
    """
    Args:
      ...
      inference_only:
    """
    self.work_dir = config.saver.output_dir
  ...
  def _setup_env(self):
    ...
    init_log('global', level=logging.INFO)
    self.logger = logging.getLogger('global')

    if self.rank == 0 and not self.inference_only:
      log_dir = os.path.join(self.work_dir, self.config.saver.log_dir)
      self.logger = add_log_file_handler(self.logger, log_dir, level=logging.INFO)
  ...
  def get_batch(self, batch_type='train', return_name=False):
    ...
    try:
      ...
      iterator = self.data_iterators[batch_type] = iter(self.data_loaders[batch_type])
      batch = next(iterator) # name, video, label
  
    output = [batch[1].cuda(non_blocking=True), batch[2].cuda(non_blocking=True)]
    if return_name:
  ...
  def train(self):
    ...
    for iter_idx in range(self.start_iter, self.max_iter):
      ...
      if (iter_idx == self.max_iter - 1) or (iter_idx % self.epoch_iters == 0 and iter_idx > 0 and \
        ...
        metric  = self.evaluate(setname='val')
  ...
  @torch.no_grad()
  def evaluate(self, setname='test', outfile=None):
    ...
    test_loader = self.data_loaders[setname]
    test_len = len(test_loader)
    end = time.time()

    outs = {}
    for iter_idx in range(test_len):
      inputs = self.get_batch(setname, return_name=False)
      ...
      if outfile is not None:
        for bname, blogit in zip(inputs[0], output.data.cpu().numpy()):
          outs[bname] = blogit
